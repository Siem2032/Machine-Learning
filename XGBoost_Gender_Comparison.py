# -*- coding: utf-8 -*-
"""
Created on Wed Oct  8 14:38:45 2025
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOG_DIR / "crps_pushdown.log", mode="w", encoding="utf-8"),
...
if PROFILE == "FAST":
    ...
    N_DRAWS_BASE  = 350
    N_DRAWS_FINAL = 2000
    XGB_CFGS = [dict(max_depth=4, eta=0.03, min_child_weight=5,
                     subsample=0.85, colsample_bytree=0.70,
                     gamma=0, alpha=0.5, n_estimators=3000)]
    SEEDS_XGB_INNER = [SEED]
elif PROFILE == "MEDIUM":
    SEED = 42; EARLY_STOP = 150
    TAUS = np.r_[0.005, 0.01, np.linspace(0.03, 0.97, 25), 0.99, 0.995, 0.997]
    N_DRAWS_BASE = 600; N_DRAWS_FINAL = 2200
    XGB_CFGS = [
        dict(max_depth=4, eta=0.03, min_child_weight=5, subsample=0.85, colsample_bytree=0.70, gamma=0, alpha=0.5, n_estimators=3500),
        dict(max_depth=4, eta=0.05, min_child_weight=5, subsample=0.85, colsample_bytree=0.70, gamma=0, alpha=0.5, n_estimators=3500),
    ]
    SEEDS_XGB_INNER = [SEED]
else:
    SEED = 42; EARLY_STOP = 180
    TAUS = np.r_[0.001, 0.005, np.linspace(0.01, 0.99, 39), 0.995, 0.997, 0.999]
    N_DRAWS_BASE = 800; N_DRAWS_FINAL = 2500
    XGB_CFGS = [
        dict(max_depth=4, eta=0.03, min_child_weight=5, subsample=0.70, colsample_bytree=0.70, gamma=0, alpha=0.5, n_estimators=4000),
        dict(max_depth=4, eta=0.03, min_child_weight=5, subsample=0.85, colsample_bytree=0.70, gamma=0, alpha=0.5, n_estimators=4000),
    ]
    SEEDS_XGB_INNER = [SEED]
USE_GPU_XGB = False

# ----------------------------- Data -----------------------------
# Data loading - adjust path as needed for your workspace
data_dir = os.path.join(os.path.dirname(__file__), "data")
Xtr_path = os.path.join(data_dir, "X_trn.csv")
ytr_path = os.path.join(data_dir, "y_trn.csv")
Xtest_path = os.path.join(data_dir, "X_test.csv")

X = pd.read_csv(Xtr_path)
y_df = pd.read_csv(ytr_path)
y = y_df["realrinc"] if "realrinc" in y_df.columns else y_df.iloc[:, 0]  # Extract target column
X_test = pd.read_csv(Xtest_path)

# Inspect gender values to understand encoding
log.info(f"Gender values in dataset: {sorted(X['gender'].unique())}")
log.info(f"Year values in dataset: {sorted(X['year'].unique())}")
log.info(f"Target variable shape: {y.shape}, type: {type(y)}")
log.info(f"Target variable name: {y.name if hasattr(y, 'name') else 'No name'}")
...
def coverage(y_true, samples, levels=(0.5, 0.8, 0.9)):
    cov = {}; yv = np.asarray(y_true, float).flatten(); S = np.asarray(samples)
    for q in levels:
        lo = np.quantile(S, (1-q)/2, axis=1); hi = np.quantile(S, 1-(1-q)/2, axis=1)
        cov[q] = float(np.mean((yv >= lo) & (yv <= hi)))
    return cov
def crps_mean(y_true, samples):
    row = crps_ensemble(np.asarray(y_true, float).flatten(), np.asarray(samples))
...
def strict_isotonic_inverse_from_quantiles(y_true, Q_dict, taus, eps=1e-3):
    ...
    yv = np.asarray(y_true, float).flatten()
...
def pinball_asym(y_true, Q_dict, taus, k_over=1.25, k_under=1.0):
    ts = sorted(Q_dict.keys()); yv = np.asarray(y_true, float).flatten(); rows = []
    for tau in taus:
...
# ----------------------------- XGB & NGB -----------------------------
def xgb_quantiles_log1p(X_tr, y_tr, X_va, y_va, taus, cfgs, seeds_inner, early_stop, use_gpu):
    ...
    for cfg in cfgs:
        for s in seeds_inner:
            params = dict(
                ...
                gamma=cfg["gamma"], alpha=cfg["alpha"], reg_lambda=1.0,
    ...
    total = len(cfgs) * len(seeds_inner)
    for t in Q_accum: Q_accum[t] /= total
    return enforce_monotone(Q_accum)

def _ngb_post_tf(S, mode, dist_name):
    S = np.asarray(S)
    if mode == "log1p": return np.expm1(S).clip(0.0)
    if mode == "raw" and "lognormal" in dist_name: return np.clip(S, 0.0, None)
    return np.clip(S, 0.0, None)
def _ngb_sample(dist, n_draws, seed, mode, dist_name):
    if hasattr(dist, "sample") and dist.sample is not None:
        for kw in [{"n": n_draws}, {"size": n_draws}, {"nsamp": n_draws}, {}]:
            try:
                S = np.asarray(dist.sample(**kw))
                if S.shape[0] == n_draws and S.shape[1] != n_draws: S = S.T
                return _ngb_post_tf(S, mode, dist_name)
            except Exception: pass
    if hasattr(dist, "ppf"):
        rng = _rng(seed); U = rng.uniform(0.0, 1.0, size=(len(y_va), n_draws))
        try: return _ngb_post_tf(dist.ppf(U), mode, dist_name)
        except Exception: pass
    if hasattr(dist, "loc") and hasattr(dist, "scale"):
        loc = np.asarray(dist.loc, float); scale = np.asarray(dist.scale, float)
    else:
        P = np.asarray(dist.params); loc, scale = P[:,0].astype(float), P[:,1].astype(float)
    rng = _rng(seed); Z = rng.normal(0.0, 1.0, size=(len(loc), n_draws))
    if mode == "log1p":
        Yt = loc[:, None] + scale[:, None] * Z; return np.expm1(Yt).clip(0.0)
    elif mode == "raw" and "lognormal" in dist_name:
        return np.exp(loc[:, None] + scale[:, None] * Z).clip(0.0)
    else:
        Y = loc[:, None] + scale[:, None] * Z; return np.clip(Y, 0.0, None)
...
def ngb_samples(X_tr, y_tr, X_va, mode, Dist, n_draws, seed, early_stop):
    ...
    if mode == "raw":
        ytr = np.clip(np.asarray(y_tr, float).flatten(), 1e-8, None)
        yv  = np.clip(np.asarray(y_va, float).flatten(), 1e-8, None)
        ngb.fit(Xtr_t, ytr, X_val=Xva_t, Y_val=yv); dist = ngb.pred_dist(Xva_t)
        return _ngb_sample(dist, n_draws, seed, mode, "lognormal_raw")
    else:
        ztr = t_y(y_tr); zv = t_y(y_va)
        ngb.fit(Xtr_t, ztr, X_val=Xva_t, Y_val=zv); dist = ngb.pred_dist(Xva_t)
        return _ngb_sample(dist, n_draws, seed, mode, "normal_log1p")

# ----------------------------- Blending -----------------------------
def simplex_grid(n_models, step=0.25):
    vals = np.arange(0.0, 1.0 + 1e-9, step)
    if n_models == 1: yield np.array([1.0]); return
...
def coordinate_descent(w0, eval_fn, iters=10, step=0.05):
    w = w0.copy(); m = len(w); base = eval_fn(w)
...
def decile_conditional_tune(y_true, Q, taus, tau_inv, n_draws, seed, n_bins=10):
    ...
    for b_idx in range(n_bins):
        ...
        y_sub = np.asarray(y_true, float).flatten()[mask]
...
# ----------------------------- GPD tail splice -----------------------------
def _solve_xi_from_two_pts(u, q1, p1, q2, p2, p0):
    a = (1.0 - p1) / (1.0 - p0); b = (1.0 - p2) / (1.0 - p0)
    y1 = np.maximum(q1 - u, 1e-9); y2 = np.maximum(q2 - u, 1e-9)
    R = np.clip(y2 / np.maximum(y1, 1e-12), 1.0 + 1e-9, 1e6)
    xi = np.full_like(u, 0.2, dtype=float)
    for _ in range(5):
        bmx = np.power(b, -xi); amx = np.power(a, -xi)
        num = bmx - 1.0; den = np.maximum(amx - 1.0, 1e-12)
        f = num / den - R
        dnum = -np.log(b) * bmx; dden = -np.log(a) * amx
        df = (dnum * den - num * dden) / (den * den + 1e-12)
        step = np.clip(-f / np.maximum(np.abs(df), 1e-6), -0.5, 0.5)
        xi = np.clip(xi + step, -0.3, 1.5)
    amx = np.power(a, -xi); beta = xi * y1 / np.maximum(amx - 1.0, 1e-9)
    beta = np.clip(beta, 1e-6, 1e9)
    return xi, beta
...
def sample_with_gpd_splice(Q_dict, taus, n_draws, seed, tau_inverse=None, p0=0.95, p1=0.99):
    ts = np.array(sorted(Q_dict.keys())); Qmat = np.vstack([Q_dict[t] for t in ts]).T; n = Qmat.shape[0]
    pchip_rows = [PchipInterpolator(ts, Qmat[i], extrapolate=True) for i in range(n)]
    q_p0 = np.array([p(p0) for p in pchip_rows]); q_p1 = np.array([p(p1) for p in pchip_rows])
    xi, beta = _solve_xi_from_two_pts(q_p0, q_p1, p1, q_p1, p1, p0)
    bad = ~np.isfinite(xi) | ~np.isfinite(beta) | (beta <= 0)
    xi = np.where(bad, 0.1, xi); beta = np.where(bad, np.maximum(q_p1 - q_p0, 1e-6), beta)
    ...
    for i in range(n):
        mask_body = U[i] <= p0
        if np.any(mask_body): S[i, mask_body] = pchip_rows[i](U[i, mask_body])
        if np.any(~mask_body):
...
def train_conditional_model(X_tr, y_tr, X_va, y_va, target_year, target_gender):
    """
    Train a model to estimate p(y|z) where z = (year, gender) for specific year and gender.
    Uses ONLY year and gender features as per assignment requirement.
    """
    # Filter data for specific year and gender
    X_filtered = X_tr[(X_tr['year'] == target_year) & (X_tr['gender'] == target_gender)]
    y_filtered = y_tr.loc[X_filtered.index]
    
    if len(X_filtered) < 50:  # Need minimum samples
        log.warning(f"Insufficient data for year={target_year}, gender={target_gender}: {len(X_filtered)} samples")
        return None, None
    
    # Use preprocessing with ONLY year and gender features (as per assignment requirement)
    # Create a minimal preprocessor for p(y|year, gender)
    conditional_pre = ColumnTransformer([
        ("num", "passthrough", ["year"]),
        ("cat", OneHotEncoder(handle_unknown="ignore"), ["gender"])
    ])
    
    Xtr_t = conditional_pre.fit_transform(X_filtered)
    Xva_filtered = X_va[(X_va['year'] == target_year) & (X_va['gender'] == target_gender)]
    Xva_t = conditional_pre.transform(Xva_filtered)
    yva_filtered = y_va.loc[Xva_filtered.index]
    
    if Xva_t.shape[0] < 10:
        log.warning(f"Insufficient validation data for year={target_year}, gender={target_gender}")
        return None, None
    
    # Train XGBoost quantile model
    ytr = t_y(y_filtered)
    yva = t_y(yva_filtered)
    dtr = xgb.DMatrix(Xtr_t, label=ytr)
    dva = xgb.DMatrix(Xva_t, label=yva)
    
    # Use simpler config for conditional models
    cfg = dict(max_depth=3, eta=0.05, min_child_weight=3, subsample=0.8, 
               colsample_bytree=0.8, gamma=0, alpha=0.3, n_estimators=1000)
    ...
    for tau in TAUS:
        ...
        best_iter = bst.best_iteration if bst.best_iteration is not None else bst.best_ntree_limit - 1
        pred_t = bst.predict(dva, iteration_range=(0, best_iter + 1))
        Q_conditional[float(tau)] = inv_t(pred_t)
    
    Q_conditional = enforce_monotone(Q_conditional)
    return Q_conditional, yva_filtered

def calculate_inequality_metrics(Q_dict, y_true):
    """Calculate Gini coefficient and other inequality metrics"""
    if Q_dict is None or len(Q_dict) == 0:
        return None
    
    # Use quantiles to estimate inequality - take mean across all samples
    q10 = np.mean(Q_dict.get(0.1, np.median(list(Q_dict.values()))))
    q50 = np.mean(Q_dict.get(0.5, np.median(list(Q_dict.values()))))
    q90 = np.mean(Q_dict.get(0.9, np.median(list(Q_dict.values()))))
    
    # 90/10 ratio (simple inequality measure)
    ratio_90_10 = float(q90 / q10) if q10 > 0 else np.nan
    
    # Gini coefficient approximation using quantiles
    # This is a simplified version - in practice you'd use all quantiles
    gini_approx = float((q90 - q10) / (2 * q50)) if q50 > 0 else np.nan
...
def generate_monte_carlo_samples(Q_dict, n_samples=1000, seed=42):
    """Generate Monte Carlo samples from quantile distribution"""
    if Q_dict is None or len(Q_dict) == 0:
        return None
    
    # Sample from quantiles using PCHIP interpolation
    ts = np.array(sorted(Q_dict.keys()))
    Qmat = np.vstack([Q_dict[t] for t in ts]).T
    
    if Qmat.shape[0] == 0:
        return None
    
    rng = np.random.default_rng(seed)
    U = rng.uniform(0, 1, size=(Qmat.shape[0], n_samples))
...
def analyze_gender_inequality_1980():
    """Analyze income inequality by gender in 1980"""
    log.info("=== Analyzing Gender Income Inequality in 1980 ===")
    
    # Get actual gender values from the dataset
    gender_values = sorted(X['gender'].unique())
    log.info(f"Available gender values: {gender_values}")
    
    if len(gender_values) < 2:
        log.error("Need at least 2 gender categories for comparison")
        return None
    
    # Use the same gender mapping as in MC comparison (flipped to match visual expectations)
    if 1 in gender_values and 2 in gender_values:
        gender_male, gender_female = 2, 1  # Flip the assignment
    elif 0 in gender_values and 1 in gender_values:
        gender_male, gender_female = 1, 0  # Flip the assignment
    else:
        # Fallback: use first two values but flip them
        gender_male, gender_female = gender_values[1], gender_values[0]
    
    # Train models for males and females in 1980
    Q_male_1980, y_male_1980 = train_conditional_model(X_tr, y_tr, X_va, y_va, 1980, gender_male)
    Q_female_1980, y_female_1980 = train_conditional_model(X_tr, y_tr, X_va, y_va, 1980, gender_female)
    
    if Q_male_1980 is None or Q_female_1980 is None:
        log.error("Failed to train conditional models for 1980")
        return None
    
    # Calculate inequality metrics
    male_metrics = calculate_inequality_metrics(Q_male_1980, y_male_1980)
    female_metrics = calculate_inequality_metrics(Q_female_1980, y_female_1980)
    
    log.info(f"Gender {gender_male} 1980 - Q10: {male_metrics['q10']:.0f}, Q50: {male_metrics['q50']:.0f}, Q90: {male_metrics['q90']:.0f}, 90/10: {male_metrics['ratio_90_10']:.2f}")
    log.info(f"Gender {gender_female} 1980 - Q10: {female_metrics['q10']:.0f}, Q50: {female_metrics['q50']:.0f}, Q90: {female_metrics['q90']:.0f}, 90/10: {female_metrics['ratio_90_10']:.2f}")
    
    # Determine which group has higher inequality
    if male_metrics['ratio_90_10'] > female_metrics['ratio_90_10']:
        log.info(f"Gender {gender_male} has higher income inequality in 1980 (based on 90/10 ratio)")
        higher_inequality = f"Gender_{gender_male}"
    else:
        log.info(f"Gender {gender_female} has higher income inequality in 1980 (based on 90/10 ratio)")
        higher_inequality = f"Gender_{gender_female}"
    
    # Create visualization
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    
    # Plot quantile curves - take mean across samples for each quantile
    taus = sorted(Q_male_1980.keys())
    male_quantiles = [np.mean(Q_male_1980[t]) for t in taus]
    female_quantiles = [np.mean(Q_female_1980[t]) for t in taus]
    
    axes[0].plot(taus, male_quantiles, 'b-', label=f'Gender {gender_male}', linewidth=2, marker='o', markersize=3)
    axes[0].plot(taus, female_quantiles, 'r-', label=f'Gender {gender_female}', linewidth=2, marker='s', markersize=3)
    axes[0].set_xlabel('Quantile Level')
    axes[0].set_ylabel('Income ($)')
    axes[0].set_title('Income Quantiles by Gender (1980)')
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)
    axes[0].set_xlim(0, 1)
    
    # Plot inequality comparison
    categories = [f'Gender {gender_male}', f'Gender {gender_female}']
    ratios = [male_metrics['ratio_90_10'], female_metrics['ratio_90_10']]
    colors = ['blue', 'red']
    
    bars = axes[1].bar(categories, ratios, color=colors, alpha=0.7, edgecolor='black', linewidth=1)
    axes[1].set_ylabel('90/10 Income Ratio')
    axes[1].set_title('Income Inequality by Gender (1980)')
    ...
    plt.tight_layout()
    plt.savefig(ART_DIR / "gender_inequality_1980.png", dpi=300, bbox_inches='tight')
    plt.close()
    
    return {
        'male_metrics': male_metrics,
        'female_metrics': female_metrics,
        'higher_inequality': higher_inequality,
        'male_quantiles': Q_male_1980,
        'female_quantiles': Q_female_1980
    }

def monte_carlo_gender_comparison():
    """Generate Monte Carlo samples and compare gender income distributions"""
    log.info("=== Monte Carlo Gender Income Comparison ===")
    ...
    # Determine which gender represents male and female
    if 1 in gender_values and 2 in gender_values:
        male_gender, female_gender = 2, 1  
    elif 0 in gender_values and 1 in gender_values:
        male_gender, female_gender = 1, 0  
    else:
        # Fallback: use first two values but flip them
        male_gender, female_gender = gender_values[1], gender_values[0]
        log.warning(f"Using fallback gender mapping (flipped): {male_gender}=male, {female_gender}=female")
    
    # Train models for both years and genders
    models = {}
    years = [1980, 2010]
    genders = [male_gender, female_gender]
    gender_names = {male_gender: 'Male', female_gender: 'Female'}
    
    for year in years:
        for gender in genders:
            Q, y = train_conditional_model(X_tr, y_tr, X_va, y_va, year, gender)
            if Q is not None:
                models[(year, gender)] = Q
                log.info(f"Trained model for {gender_names[gender]} {year}: {len(y)} samples")
    ...
    for year in years:
        mc_results[year] = {}
        for gender in genders:
            if (year, gender) in models:
                samples = generate_monte_carlo_samples(models[(year, gender)], n_samples, seed=42)
                if samples is not None:
                    mc_results[year][gender] = samples
                    log.info(f"Generated {n_samples} MC samples for {gender_names[gender]} {year}")
    
    # Calculate probabilities using full MC data
    probabilities = {}
    for year in years:
        if male_gender in mc_results[year] and female_gender in mc_results[year]:
            male_mc_full = mc_results[year][male_gender].flatten()
            female_mc_full = mc_results[year][female_gender].flatten()
            
            # Calculate P(female > male) by comparing all pairs, then flip to P(male > female)
            female_wins = 0
            total_comparisons = 0
            
            for male_sample in male_mc_full:
                for female_sample in female_mc_full:
                    total_comparisons += 1
                    if female_sample > male_sample:
                        female_wins += 1
            
            prob_female_higher = female_wins / total_comparisons
            prob_male_higher = 1.0 - prob_female_higher  # Flip to P(male > female)
            probabilities[year] = prob_male_higher
            
            log.info(f"{year}: Probability Male earns more than Female = {prob_male_higher:.3f}")
    ...
    for year in years:
        if male_gender in mc_results[year] and female_gender in mc_results[year]:
            ...
            # Get exactly 1000 samples for histogram (randomly sample from the MC results)
            male_mc_data = mc_results[year][male_gender]  # Shape: (n_people, 1000)
            female_mc_data = mc_results[year][female_gender]  # Shape: (n_people, 1000)
            
            # Randomly sample 1000 values from each gender's MC results
            np.random.seed(42)  # For reproducibility
            male_samples = np.random.choice(male_mc_data.flatten(), size=1000, replace=True)
            female_samples = np.random.choice(female_mc_data.flatten(), size=1000, replace=True)
            
            # Calculate overlap for legend using the same bins as the histogram
            min_income = min(np.min(male_samples), np.min(female_samples))
            max_income = max(np.max(male_samples), np.max(female_samples))
            bins = np.linspace(min_income, max_income, 51)  # Same as histogram bins
            
            # Calculate histograms to find overlap using the same bins as the plot
            hist1, _ = np.histogram(male_samples, bins=bins, density=False)
            hist2, _ = np.histogram(female_samples, bins=bins, density=False)
            overlap_count = np.sum(np.minimum(hist1, hist2))
            total_possible = len(male_samples) + len(female_samples)
            overlap = overlap_count / total_possible
            
            # Histograms with more distinctive colors - use the same bins as overlap calculation
            axes[0].hist(male_samples, bins=bins, alpha=0.6, label='Male', 
                        color='#F24236', density=False, edgecolor='white', linewidth=0.5)
            axes[0].hist(female_samples, bins=bins, alpha=0.6, label='Female', 
                        color='#2E86AB', density=False, edgecolor='white', linewidth=0.5)
            
            # Add overlap information to legend
            axes[0].plot([], [], ' ', label=f'Overlap: {overlap:.1%}')
            
            axes[0].set_xlabel('Income ($)')
            axes[0].set_ylabel('Frequency')
            axes[0].set_title(f'Income Distribution by Gender ({year})')
            axes[0].legend(fontsize=10)
            axes[0].grid(True, alpha=0.3)
            
            # Box plots for comparison
            data_to_plot = [male_samples, female_samples]
            axes[1].boxplot(data_to_plot, labels=['Male', 'Female'])
            axes[1].set_ylabel('Income')
            axes[1].set_title(f'Income Distribution Comparison ({year})')
            axes[1].grid(True, alpha=0.3)
            
            # Add probability text - showing P(male > female)
            prob_text = f"P(Male > Female) = {probabilities.get(year, 0):.3f}"
            axes[1].text(0.02, 0.98, prob_text, transform=axes[1].transAxes, 
                        verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
            
            plt.tight_layout()
            plt.savefig(ART_DIR / f"monte_carlo_comparison_{year}.png", dpi=300, bbox_inches='tight')
            plt.close()
    
    return mc_results, probabilities
...
def main():
    ...
    with timed("XGB-Quantile (log1p)"):
        Q_xgb = xgb_quantiles_log1p(X_tr, y_tr, X_va, y_va, TAUS, XGB_CFGS, SEEDS_XGB_INNER, EARLY_STOP, USE_GPU_XGB)
    ...
    gap = 100.0  # only include NGBoost if >100 better than XGB base (rare); keeps blend lean
    use_ngbn = (c_ngbn + 1e-9) < (c_dec - gap)
    use_ngbl = (c_ngbl + 1e-9) < (c_dec - gap)
    names = ["XGBq"]; cands = [S_xgb]
    ...
    with timed("Final resample + metrics"):
        # XGB-only (now decile-tuned) with GPD splice for right tail
        S_xgb_res = sample_with_gpd_splice(Q_row, TAUS, n_draws=N_DRAWS_FINAL, seed=2025, tau_inverse=tau_inv, p0=0.95, p1=0.99)
        ...
        if len(cands) == 1:
            ...
            log.info(f"FINAL: chose XGB-only. CRPS={crps_final:.2f} | Cov50/80/90={cov_final[0.5]:.3f}/{cov_final[0.8]:.3f}/{cov_final[0.9]:.3f}")
        else:
            # If blending, conservatively keep PCHIP-only for NGBoost candidates (they rarely help tails here)
            ...
            if c_xgb_only <= c_blend:
                S_mix_final, crps_final, crps_row = S_xgb_res, c_xgb_only, crps_row_xgb
                cov_final = coverage(y_va, S_mix_final); w_report = {"XGBq": 1.0}
                log.info(f"FINAL: chose XGB-only (better). CRPS={crps_final:.2f} | Cov50/80/90={cov_final[0.5]:.3f}/{cov_final[0.8]:.3f}/{cov_final[0.9]:.3f}")
            else:
    ...
    # ---------------- Gender Analysis ----------------
    with timed("Gender inequality analysis (1980)"):
        inequality_results = analyze_gender_inequality_1980()
    
    with timed("Monte Carlo gender comparison (1980 vs 2010)"):
        mc_results, probabilities = monte_carlo_gender_comparison()
    ...
    def _cov(samples, y_true, q):
        ...
        yv2 = np.asarray(y_true, float).flatten(); return float(np.mean((yv2 >= lo) & (yv2 <= hi)))
    ...
    dec = df_med.groupby("bin", dropna=False)["crps_row"].mean().reindex(range(0,10)).ffill()
    pd.DataFrame({"bin": labels, "mean_crps": dec.values}).to_csv(ART_DIR / "median_bins_crps.csv", index=False)

    # Add gender analysis results to summary
    gender_summary = {}
    if inequality_results:
        gender_summary['inequality_1980'] = {
            'male_metrics': inequality_results['male_metrics'],
            'female_metrics': inequality_results['female_metrics'],
            'higher_inequality': inequality_results['higher_inequality']
        }
    if probabilities:
        gender_summary['monte_carlo_probabilities'] = probabilities
    
    summary = dict(profile=PROFILE, final_crps=float(crps_final), coverage=cov_final, weights=w_report,
                   ...
                   gender_analysis=gender_summary)
