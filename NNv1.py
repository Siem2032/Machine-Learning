# CRPS WAS 9500 HIER, WEL ALLEEN MAAR LOG-NORMAL DISTRIBUTION
import pandas as pd
import numpy as np
...
def log_normal_nll(y, mu, sigma):
    ...
    nll = torch.log(y + 1e-8) + 0.5 * torch.log(2 * torch.pi * sigma**2) + 0.5 * ((logy - mu) ** 2) / (sigma**2)
    return nll.mean()
...
with torch.no_grad():
    for numeric, onehot, educcat, occrecode, y in val_loader:
        numeric, onehot, educcat, occrecode, y = (
            numeric.to(device), onehot.to(device), educcat.to(device), occrecode.to(device), y.to(device)
        )
        mu, sigma = model(numeric, onehot, educcat, occrecode)
        
        eps = torch.randn(mu.size(0), 1000, device=device)
        samples = torch.exp(mu.unsqueeze(1) + sigma.unsqueeze(1) * eps)  # log-normal samples
        val_samples.append(samples.cpu())
        y_val_array.append(y.cpu())

val_pred = torch.cat(val_samples, dim=0).numpy()   # shape (n_val, 1000)
y_val_array = torch.cat(y_val_array, dim=0).numpy()  # echte targets

crps_val = crps_ensemble(y_val_array, val_pred).mean().item()
print("Validation CRPS:", crps_val)

## Make Pedictions y for the test set
# --- 10. Preprocess test set op exact dezelfde manier als training ---
X_test_proc = X_test.copy()

# 1. Encode educcat (met dezelfde encoder als training)
X_test_proc["educcat"] = educcat_encoder.transform(X_test_proc[["educcat"]]).astype(int)

# 2. One-hot encode (reindex zodat kolommen matchen met training)
...
# 4. Standaardiseer numerieke features
...
# --- 12. Monte Carlo samples per observatie ---
model.eval()
all_samples = []

with torch.no_grad():
    for numeric, onehot, educcat, occrecode in test_loader:
        numeric, onehot, educcat, occrecode = numeric.to(device), onehot.to(device), educcat.to(device), occrecode.to(device)
        mu, sigma = model(numeric, onehot, educcat, occrecode)

        # 1000 samples per observation
        eps = torch.randn(mu.size(0), 1000, device=device)
        samples = torch.exp(mu.unsqueeze(1) + sigma.unsqueeze(1) * eps)  # log-normal sampling
        all_samples.append(samples.cpu())

y_pred = torch.cat(all_samples, dim=0).numpy()  # shape (n_obs, 1000)

print("Prediction matrix shape:", y_pred.shape)  # moet (5578, 1000) zijn
